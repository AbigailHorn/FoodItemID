...
y.lab.in <- "$B^{sim}_{N_i}$"
y.lim.in <- c(.1156,.11585)
plot.mean=0
p_singleOB <- plot.together.ribbon(data.in=data.in,vars.to.plot=vars.to.plot,CI.lvl=CI.lvl, 
                                        y.lab.in=y.lab.in, y.lim.in=y.lim.in,plot.mean=plot.mean)
#p_singleOB

```

<!--- Print this single outbreak --->

```{r}

#pdf(file = path(output.dir, "baseline2/BaselineSigSIM.pdf"), width=10, height =10)
p_singleOB
#dev.off()
...
#pdf(file = path(output.dir, "baseline2/BaselineSigRAND_zoomIN.pdf"), width=10, height =10)
p_singleOB
#dev.off()
