...
%ill_over_time_interval = [20:20:100 150:50:300];

ill_over_time_interval = [20:20:100 150:50:300 400:200:1000];  % EHEC
%ill_over_time_interval = [1 10 20:20:60];  % Listeria
...
if both_norm == 1
    for norm_method = 1:2
        ...
        [all_net_data, net_MC_means_NormSig, net_MC_CI95_NormSig] = plot_compute_signal_rawvsSimSig_WHS(all_food_nets, all_net_names, prior_pmf, stage_ends, current_ob_data, current_ob_names, ill_over_time_interval, MC_method, num_samples, norm_method, save_plot);
    end
else
    norm_method = dpr_flag;
    [all_net_data, net_MC_means_RawSig, net_MC_CI95_RawSig] = plot_compute_signal_rawvsSimSig_WHS(all_food_nets, all_net_names, prior_pmf, stage_ends, current_ob_data, current_ob_names, ill_over_time_interval, MC_method, num_samples, norm_method, save_plot);
end

all_net_data_EHEC = all_net_data;

foldername = '/Users/abigailhorn/Dropbox/MATLAB/Elena2017/utility/Model_Comparison/0_Dec2020/Results/ACC_tests';
filename = sprintf('%s/convergence_singleOB_EHEC.csv',foldername);
writematrix(all_net_data_EHEC, filename);
