...
function [all_net_data, net_MC_means_raw, net_MC_CI95_raw] = plot_compute_signal_rawvsSimSig_WHS(all_food_nets, all_net_names, init_vals, stage_ends, all_ob_data, all_ob_names, ill_over_time_interval, MC_method, num_samples, dpr_flag, save_plot)
%function [nested_net_data, net_MC_means_NormSig, net_MC_CI95_NormSig] = plot_compute_signal_rawvsSimSig_WHS(all_food_nets, all_net_names, init_vals, stage_ends, all_ob_data, all_ob_names, ill_over_time_interval, MC_method, num_samples, dpr_flag, save_plot)
...
all_net_data = zeros(num_time_points, num_food_nets * num_samples);
...
%                [~, MC_SimSig, MC_ratio_raw] = MC_compute_PracticalUB_RLG(method_i, food_net, stage_ends, prior_pmf, current_ob_data, num_samples, ill_over_time_interval);
                 [~, MC_NormSig, MC_ratio_raw] = SimSig_L3_OB_WHS4(net_i, method_i, food_net, stage_ends, prior_pmf, current_ob_data, num_samples, ill_over_time_interval);
             ...
             %%%%%
                ...
                [~, MC_NormSig, MC_ratio_raw] = MC_compute_DPR_PractialUB_RLG(method_i, food_net, stage_ends, prior_pmf, current_ob_data, num_samples, ill_over_time_interval);
            ...
            all_net_data(:,((num_samples*(net_i - 1))+1):(num_samples*net_i))  =  MC_NormSig; %[all_net_data, MC_NormSig];
            %nested_net_data{net_i,1} = MC_NormSig;
            ...
            y = MC_NormSig.'; %randn(50,100);                                  % Create Dependent Variable ?Experiments? Data
            N = size(y,1);                                      % Number of ?Experiments? In Data Set
            yMean = mean(y);                                    % Mean Of All Experiments At Each Value Of ?x?
            ySEM = std(y)/sqrt(N);                              % Compute ?Standard Error Of The Mean? Of All Experiments At Each Value Of ?x?
            CI95 = tinv([0.025 0.975], N-1);                    % Calculate 95% Probability Intervals Of t-Distribution
            yCI95 = bsxfun(@times, ySEM, CI95(:));              % Calculate 95% Confidence Intervals Of All Experiments At Each Value Of ?x?
            % put result into matrix
            net_MC_CI95_NormSig(:,net_i) = yCI95(2,:);
...
%         plot_MC_CI95_RawOrSimSig("SimSig", ob_name, all_net_names, method_i, num_samples, net_MC_means_SimSig, net_MC_CI95_SimSig, ill_over_time_interval, dpr_flag, save_plot)
        ...
        plot_MC_CI95_RawOrSimSig("Raw", ob_name, all_net_names, method_i, num_samples, net_MC_means_raw, net_MC_CI95_raw, ill_over_time_interval, dpr_flag, save_plot)
        plot_MC_CI95_RawOrSimSig("SimSig", ob_name, all_net_names, method_i, num_samples, net_MC_means_NormSig, net_MC_CI95_NormSig, ill_over_time_interval, dpr_flag, save_plot)
