function [Figure] = plot_MC_CI95_RawOrSimSig(rawVSsig, ob_name, all_net_names, method_i, num_samples, net_MC_means, net_MC_CI95, ill_over_time_interval, dpr_flag, save_plot)


%%
num_nets = size(net_MC_means,2);
...
for net_i = 1:num_nets
    MC_mean = net_MC_means(:,net_i).';
    plot(x, MC_mean,'LineWidth',1, 'Color', C(net_i,:));% 'Color','w')
end %over net_i

for net_i = 1:num_nets
    MC_mean = net_MC_means(:,net_i).';
    MC_CI95 = net_MC_CI95(:,net_i).';
...
hTitle  = title (sprintf('Norm by: %s || Method: %s || OB: %s || iter: %0.0f || %s ', normalize, MC_method, ob_name,num_samples, rawVSsig))

title (sprintf('Norm by: %s || Method: %s || OB: %s || iter: %0.0f || %s ', normalize, MC_method, ob_name,num_samples,rawVSsig))
%title('Norm by: OB || Method: KLD || OB: SimulatedOB-Veg || iter:50')

hXLabel = xlabel('Number ill');
hYLabel_left = ylabel('Norm-Sig Signal Strength');
...
if save_plot == 1
    ...
    filename = '/Users/abigailhorn/Dropbox/MATLAB/Elena2017/utility/Model_Comparison/0_Dec2020/Results2021/Real_OB';
    eval(sprintf(  'print -djpeg100 %s/OB%s_method%0.0f_norm%s_%s',filename,ob_name,method_i,normalize,rawVSsig));
end
